...
class DualPhoneCalibrationCaptureStore(context: Context) {
    ...
    fun saveAcceptedFrame(
        ...
        val framesDir = File(roleRoot, "accepted_frames").apply { mkdirs() }
        val output = File(
            framesDir,
        ...
        for (index in entries.length() - 1 downTo 0) {
            if (entries.optJSONObject(index)?.optInt("pose_index", -1) == poseIndex) {
        ...
        val entry = JSONObject()
            ...
            .put("pose_id", poseId)
            .put("file", "accepted_frames/${output.name}")
        ...
        manifest
            ...
            .put("capture_type", "dual_phone_intrinsics_samples")
