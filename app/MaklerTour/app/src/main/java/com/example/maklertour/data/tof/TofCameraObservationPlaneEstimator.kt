...
/**
 * ChArUco observation -> physical board plane in CAMERA_A coordinates.
 *
 * Unlike TofCameraCharucoPlaneEstimator this consumes the already accepted
 * calibration observation, so LM03.4B2 can persist the exact ToF sample inside
 * the calibration gate before the pose counter advances.
 */
object TofCameraObservationPlaneEstimator {
    fun estimate(
        ...
        if (observation.imageWidth <= 0 || observation.imageHeight <= 0) {
            return failed("Invalid CAMERA_A observation dimensions")
        }
        if (observation.charucoCorners.size < MIN_SOLVE_CORNERS) {
            return failed(
                "Need at least $MIN_SOLVE_CORNERS accepted ChArUco corners",
                observation.charucoCorners.size,
            )
        }
        ...
        observation.charucoCorners.forEach { corner ->
            if (corner.id !in boardCorners.indices) return@forEach
            val boardPoint = boardCorners[corner.id]
            objectPoints += Point3(boardPoint.x, boardPoint.y, boardPoint.z)
            imagePoints += Point(
                corner.normalizedX * observation.imageWidth.toDouble(),
                corner.normalizedY * observation.imageHeight.toDouble(),
            )
        }
        ...
        try {
            ...
            if (!plane.structurallyValid) {
                return failed(
                    "Invalid ChArUco CAMERA_A board plane",
                    objectPoints.size,
                )
            }
            ...
            return TofCameraBoardPlaneEstimate(
                ...
                status = "Accepted ChArUco CAMERA_A board plane solved",
            )
        } catch (error: Throwable) {
            return failed(
                "ChArUco observation solve failed: " +
    ...
    private const val MIN_SOLVE_CORNERS = 6
}
