...
# 1. Добавляем все изменения для файлов с расширениями .php, .js, .html и других
echo "==> Добавляем изменения для файлов с расширениями .php, .js, .html"
git add *.php *.js *.html
...
if git diff --cached --quiet; then
  ...
  echo "==> Добавлены новые изменения. Коммитим..."
  git commit -m "Auto commit: добавлены изменения для .php, .js, .html файлов"
  
  # Пушим изменения на удаленный репозиторий
  echo "==> Пушим изменения в ${REMOTE}/${BRANCH}"
  git push -u "${REMOTE}" "${BRANCH}"
fi

# 2. Получаем последние изменения с удаленного репозитория
echo "==> Fetch latest ${REMOTE}/${BRANCH}"
git fetch "${REMOTE}" "${BRANCH}" --prune

REMOTE_HEAD_BEFORE="$(git rev-parse "${REMOTE}/${BRANCH}")"

# 3. Pull latest changes from remote branch
