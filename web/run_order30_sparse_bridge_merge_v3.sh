...
COLMAP_BIN="${12}"
COLMAP_IMAGE="${13}"
COLMAP_CAMERA_MODEL_AUTO_FROM_METADATA="${14}"
...

{
  ...
  echo "Bridge source: $SOURCE_FRAMES"
  echo "Bridge range: $START_FRAME..$END_FRAME"
...
echo "Bridge input frames: $frame_count" | tee -a "$RUN_LOG"

if (( frame_count < 20 )); then
  echo "ERROR: bridge frame subset contains only $frame_count images" | tee -a "$RUN_LOG" >&2
  exit 2
fi
...
# process_colmap_sparse.sh redirects stderr to $STATION_BASE/logs/job_<id>.log,
# so tee around the process alone produces an empty console log.
set +e
