...
def size(p):
    ...
    return total
def safe(p):
    allowed=[base+"/input/job_",base+"/output/job_",base+"/incoming/job_",base+"/logs/job_",base+"/status/job_"]
    return any(p.startswith(a) for a in allowed) and "/../" not in p
for jid in ids:
    ...
    for p in sorted(set(candidates)):
        if not safe(p): errors.append({"path":p,"message":"unsafe path rejected"}); continue
        ...
        paths.append(rec)
print(json.dumps({"paths":paths,"errors":errors}, separators=(",",":")))
PY'
