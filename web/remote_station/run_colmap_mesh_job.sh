#!/usr/bin/env bash
set -euo pipefail
if [[ $# -lt 4 || $# -gt 8 ]]; then echo "Usage: $0 ./stations.conf <mesh_job_id> <reconstruction_parent_job_id> <preview|hq> [engine] [depth] [target_faces] [density_quantile]" >&2; exit 1; fi
CONFIG="$1"; MESH_JOB_ID="$2"; PARENT_JOB_ID="$3"; MODE="$4"; REQ_ENGINE="${5:-}"; REQ_DEPTH="${6:-}"; REQ_TARGET="${7:-}"; REQ_DQ="${8:-}"
...
DENSITY_QUANTILE="${REQ_DQ:-${MESH_DENSITY_QUANTILE:-0.07}}"
...
printf -v B '%q' "$STATION_BASE"; printf -v DQ '%q' "$DENSITY_QUANTILE"; printf -v CM '%q' "$COLMAP_MODE"; printf -v CB '%q' "$COLMAP_BIN"; printf -v CI '%q' "$COLMAP_IMAGE"; printf -v ME '%q' "$MESH_ENGINE"; printf -v OP '%q' "$OPEN3D_PYTHON"
