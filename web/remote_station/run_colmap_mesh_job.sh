...
printf -v B '%q' "$STATION_BASE"; printf -v CM '%q' "$COLMAP_MODE"; printf -v CB '%q' "$COLMAP_BIN"; printf -v CI '%q' "$COLMAP_IMAGE"
printf -v A '%q ' "$MESH_JOB_ID" "$PARENT_JOB_ID" "$MODE" "$DEPTH" "$TARGET" "$MIN_IN" "$MIN_FACES"
"${SSH[@]}" "mkdir -p '$STATION_BASE/logs'; { echo '[launcher] mesh job $MESH_JOB_ID parent $PARENT_JOB_ID mode $MODE'; STATION_BASE=$B COLMAP_MODE=$CM COLMAP_BIN=$CB COLMAP_IMAGE=$CI nohup $B/scripts/process_colmap_mesh.sh $A > '$STATION_BASE/logs/job_${MESH_JOB_ID}_mesh_launcher.log' 2>&1 & echo launched; }"