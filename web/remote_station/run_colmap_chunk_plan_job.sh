...
printf -v Q_COLMAP_BIN '%q' "${COLMAP_BIN:-colmap}"

"${SSH[@]}" "set -e; test -x $Q_PY; test -d $Q_SPARSE; mkdir -p $Q_PARENT; COLMAP_BIN=$Q_COLMAP_BIN $Q_PY '$STATION_BASE/scripts/plan_colmap_dense_chunks.py' --sparse-model-dir $Q_SPARSE --model-id $Q_MODEL --mode $Q_MODE --output-plan $Q_PLAN --target-images-per-chunk $Q_TARGET --max-images-per-chunk $Q_MAX --overlap-images $Q_OVERLAP --sparse-job-id $Q_SPARSE_ID --ram-reserve-mb $Q_RESERVE"
