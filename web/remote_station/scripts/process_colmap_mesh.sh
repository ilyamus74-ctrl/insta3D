...
OPEN3D_PYTHON="${OPEN3D_PYTHON:-$BASE/open3d-venv/bin/python}"; OPEN3D_MESH_SCRIPT="${OPEN3D_MESH_SCRIPT:-$BASE/scripts/build_clean_mesh.py}"; MESH_ENGINE="${MESH_ENGINE:-open3d}"
case "$MODE" in
  full|fullhd|1920) O3D_DEPTH="${OPEN3D_FULLHD_DEPTH:-9}"; O3D_TARGET="${OPEN3D_FULLHD_TARGET_FACES:-500000}"; O3D_DQ="${OPEN3D_FULLHD_DENSITY_QUANTILE:-0.05}"; O3D_NB="${OPEN3D_FULLHD_STATISTICAL_NB_NEIGHBORS:-30}"; O3D_STD="${OPEN3D_FULLHD_STATISTICAL_STD_RATIO:-1.8}"; O3D_RADIUS_NB="${OPEN3D_FULLHD_RADIUS_NB_POINTS:-8}" ;;
  standard|1600) O3D_DEPTH="${OPEN3D_STANDARD_DEPTH:-8}"; O3D_TARGET="${OPEN3D_STANDARD_TARGET_FACES:-250000}"; O3D_DQ="${OPEN3D_STANDARD_DENSITY_QUANTILE:-0.07}"; O3D_NB="${OPEN3D_STANDARD_STATISTICAL_NB_NEIGHBORS:-24}"; O3D_STD="${OPEN3D_STANDARD_STATISTICAL_STD_RATIO:-2.0}"; O3D_RADIUS_NB="${OPEN3D_STANDARD_RADIUS_NB_POINTS:-6}" ;;
  hq) O3D_DEPTH="${OPEN3D_HQ_DEPTH:-9}"; O3D_TARGET="${OPEN3D_HQ_TARGET_FACES:-500000}"; O3D_DQ="${OPEN3D_HQ_DENSITY_QUANTILE:-0.05}"; O3D_NB="${OPEN3D_HQ_STATISTICAL_NB_NEIGHBORS:-30}"; O3D_STD="${OPEN3D_HQ_STATISTICAL_STD_RATIO:-1.8}"; O3D_RADIUS_NB="${OPEN3D_HQ_RADIUS_NB_POINTS:-8}" ;;
  *) O3D_DEPTH="${OPEN3D_PREVIEW_DEPTH:-7}"; O3D_TARGET="${OPEN3D_PREVIEW_TARGET_FACES:-100000}"; O3D_DQ="${OPEN3D_PREVIEW_DENSITY_QUANTILE:-0.10}"; O3D_NB="${OPEN3D_PREVIEW_STATISTICAL_NB_NEIGHBORS:-20}"; O3D_STD="${OPEN3D_PREVIEW_STATISTICAL_STD_RATIO:-2.0}"; O3D_RADIUS_NB="${OPEN3D_PREVIEW_RADIUS_NB_POINTS:-5}" ;;
esac
...
run_open3d(){ status RUNNING 5 "Loading dense cloud"; set +e; "$OPEN3D_PYTHON" "$OPEN3D_MESH_SCRIPT" "$INPUT_PLY" "$OPEN3D_PLY" "$O3D_DEPTH" "$O3D_TARGET" --result-json "$RESULT_JSON.open3d" --mode "$MODE" --density-quantile "$O3D_DQ" --statistical-nb-neighbors "$O3D_NB" --statistical-std-ratio "$O3D_STD" --radius-nb-points "$O3D_RADIUS_NB" > "$LOG_DIR/open3d_mesh.log" 2>&1; local ec=$?; set -e; if [[ $ec -eq 0 ]]; then finish_result open3d "$OPEN3D_PLY" "Mesh completed"; return 0; fi; local m; m=$(python3 -c "import json;print(json.load(open('$RESULT_JSON.open3d')).get('message','Open3D failed'))" 2>/dev/null || echo Open3D failed); status ERROR 65 "Open3D mesh generation failed: $m"; cp -f "$RESULT_JSON.open3d" "$RESULT_JSON" 2>/dev/null || true; return 1; }
