...
run_colmap image_undistorter --image_path "$FRAMES_DIR" --input_path "$SPARSE_MODEL_DIR" --output_path "$UNDISTORTED_DIR" --output_type COLMAP --image_list_path "$CHUNK_DIR/image_list.txt" --max_image_size "$MAX_IMAGE_SIZE" > "$LOG_DIR/image_undistorter.log" 2>&1
status RUNNING 45 "PatchMatch chunk $CHUNK_ID"
run_colmap patch_match_stereo --workspace_path "$UNDISTORTED_DIR" --workspace_format COLMAP --PatchMatchStereo.geom_consistency true --PatchMatchStereo.cache_size "$PMC" --PatchMatchStereo.num_images "$SRC" > "$LOG_DIR/patch_match_stereo.log" 2>&1
