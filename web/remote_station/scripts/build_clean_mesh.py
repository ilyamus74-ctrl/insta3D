...
def main():
    ...
    ap.add_argument('--maximum-triangle-edge-multiplier',type=float,default=8.0)
    ap.add_argument('--component-min-triangles',type=int,default=100); ap.add_argument('--component-min-largest-ratio',type=float,default=0.01); ap.add_argument('--minimum-component-ratio',type=float,dest='component_min_largest_ratio')
    ...
    try:
        ...
        status(sf,80,'Removing long triangles');
        verts=np.asarray(mesh.vertices); tris=np.asarray(mesh.triangles); edge_threshold=med*a.maximum_triangle_edge_multiplier if med>0 else float('inf')
        if len(tris) and np.isfinite(edge_threshold):
            ...
            long_mask=(e0>edge_threshold)|(e1>edge_threshold)|(e2>edge_threshold); mesh.remove_triangles_by_mask(long_mask); cleanup(mesh); stats['long_edge_threshold']=edge_threshold; stats['triangles_removed_by_long_edge']=int(long_mask.sum())
        o3d.io.write_triangle_mesh(str(mdir/'mesh_edge_filtered.ply'),mesh)
        ...
        stats['component_min_triangles_kept']=min_keep; stats['faces_after_component_filter']=len(mesh.triangles); o3d.io.write_triangle_mesh(str(mdir/'mesh_component_filtered.ply'),mesh)
