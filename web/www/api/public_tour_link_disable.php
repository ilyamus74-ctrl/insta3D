...
$stmt=$dbcnx->prepare("SELECT cs.order_id,o.broker_id FROM capture_sessions cs JOIN tour_orders o ON o.id=cs.order_id WHERE cs.id=? LIMIT 1"); $stmt->bind_param('i',$sessionId);$stmt->execute();$row=$stmt->get_result()->fetch_assoc();$stmt->close(); if(!$row) api_json(['ok'=>false],404);
if(!($role==='ADMIN'||(int)$row['broker_id']===$uid)) api_json(['ok'=>false,'error'=>'forbidden'],403);
