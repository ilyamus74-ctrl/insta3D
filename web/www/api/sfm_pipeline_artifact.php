...
if($pipelineRunId<=0 || !in_array($artifact,['sparse','dense','mesh','result'],true)){ fail_art(400,'Bad request'); }
...
if($artifact==='result'){ $path=(string)($run['output_result_json_path'] ?? ''); $name='pipeline_'.$pipelineRunId.'_result.json'; $ctype='application/json'; }
  ...
  $sparse=null; $recon=null; $mesh=null; foreach($jobs as $j){$jt=(string)$j['job_type']; if($jt==='COLMAP_SPARSE'){$sparse=$j;} elseif(in_array($jt,['COLMAP_RECONSTRUCTION_PREVIEW','COLMAP_RECONSTRUCTION_HQ'],true)){$recon=$j;} elseif($jt==='COLMAP_MESH'){$mesh=$j;}}
  ...
  if($artifact==='dense' && $recon){ $path=remote_dir_art((int)$recon['remote_job_id']).'/merged/merged_fused.ply'; $name='pipeline_'.$pipelineRunId.'_dense_point_cloud.ply'; }
...
if($artifact!=='result'){ $pi=ply_info_art($real); if(!$pi['valid']){fail_art(404,'PLY artifact is empty or invalid');} if($artifact==='mesh' && $pi['faces']<=0){fail_art(404,'Mesh artifact has no faces');} }
