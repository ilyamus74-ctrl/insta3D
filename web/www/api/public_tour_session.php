...
$stmt=$dbcnx->prepare("SELECT cs.id,cs.order_id,cs.app_session_uuid,cs.status,o.title,o.address FROM capture_sessions cs JOIN tour_orders o ON o.id=cs.order_id WHERE cs.id=? AND cs.deleted_at IS NULL LIMIT 1");
...
$photoPoints=[]; $activePointSet=[]; $stmt=$dbcnx->prepare("SELECT id,name,room_name,sequence_number,preview_storage_path,original_storage_path,initial_yaw_deg,initial_pitch_deg,initial_hfov FROM photo_points WHERE session_id=? AND deleted_at IS NULL AND COALESCE(upload_state,'') <> 'DELETED' ORDER BY COALESCE(sequence_number,999999),created_at,id");
...
if($stmt){ $stmt->bind_param('i',$sessionId); $stmt->execute(); $rs=$stmt->get_result(); while($r=$rs->fetch_assoc()){ $fromId=(int)$r['from_photo_point_id']; $toId=(int)$r['to_photo_point_id']; if(!isset($activePointSet[$fromId]) || !isset($activePointSet[$toId])){continue;} $links[]=['id'=>(int)$r['id'],'session_id'=>$sessionId,'from_photo_point_id'=>$fromId,'to_photo_point_id'=>$toId,'yaw_deg'=>(float)$r['yaw_deg'],'pitch_deg'=>(float)$r['pitch_deg'],'target_yaw_deg'=>$r['target_yaw_deg']!==null?(float)$r['target_yaw_deg']:null,'target_pitch_deg'=>$r['target_pitch_deg']!==null?(float)$r['target_pitch_deg']:null,'target_hfov'=>$r['target_hfov']!==null?(float)$r['target_hfov']:null,'label'=>(string)($r['label']??''),'source'=>(string)($r['source']??'MANUAL'),'shared_markers'=>(json_decode((string)($r['shared_markers_json']??'[]'),true)?:[]),'confidence'=>isset($r['confidence'])?(float)$r['confidence']:null]; } $stmt->close(); }
$positions=[]; $stmt=$dbcnx->prepare("SELECT photo_point_id,x_m,y_m,z_m,yaw_deg,source FROM tour_point_positions WHERE session_id=?");
if($stmt){ $stmt->bind_param('i',$sessionId); $stmt->execute(); $rs=$stmt->get_result(); while($r=$rs->fetch_assoc()){ $pid=(int)$r['photo_point_id']; if(!isset($activePointSet[$pid])){continue;} $positions[(string)$pid]=['photo_point_id'=>$pid,'x_m'=>(float)$r['x_m'],'y_m'=>(float)$r['y_m'],'z_m'=>(float)$r['z_m'],'yaw_deg'=>(float)$r['yaw_deg'],'source'=>(string)($r['source']??'UNKNOWN')]; } $stmt->close(); }
