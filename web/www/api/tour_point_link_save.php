...
if (!is_array($data)) {
    api_json(['ok' => false, 'error' => 'bad_json'], 400);
}
...
$fromId = (int)($data['from_photo_point_id'] ?? 0);
...
if ($sessionId <= 0 || $fromId <= 0 || $toId <= 0) {
    api_json(['ok' => false, 'error' => 'bad_input'], 400);
}
if ($fromId === $toId) {
    api_json(['ok' => false, 'error' => 'same_point_forbidden'], 400);
}
...
if (!can_view_order(['broker_id' => $session['broker_id'], 'operator_id' => $session['operator_id'], 'is_published' => $session['is_published'], 'status' => $session['order_status']], $userId, $role)) {
    api_json(['ok' => false, 'error' => 'forbidden'], 403);
}

$stmt = $dbcnx->prepare("SELECT id FROM photo_points WHERE session_id = ? AND id IN (?, ?)");
if (!$stmt) {
    api_json(['ok' => false, 'error' => 'db_prepare_points_failed'], 500);
}
$stmt->bind_param('iii', $sessionId, $fromId, $toId);
$stmt->execute();
$rs = $stmt->get_result();
$count = 0;
...
$stmt->close();
if ($count < 2) {
    api_json(['ok' => false, 'error' => 'photo_points_mismatch'], 400);
}

$stmt = $dbcnx->prepare("SELECT id FROM tour_point_links WHERE session_id = ? AND from_photo_point_id = ? AND to_photo_point_id = ? LIMIT 1");
if (!$stmt) {
    api_json(['ok' => false, 'error' => 'db_prepare_find_failed'], 500);
}
$stmt->bind_param('iii', $sessionId, $fromId, $toId);
$stmt->execute();
