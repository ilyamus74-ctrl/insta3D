...
function osv_video_tof_info(int $orderId, array $session, array $video): array { $out=['active'=>false,'frames'=>0]; $dir=osv_session_videos_dir($orderId,(string)($session['app_session_uuid'] ?? '')); if(!is_dir($dir)){ return $out; } $scanId=(int)($video['id'] ?? 0); $appScanUuid=(string)($video['app_scan_uuid'] ?? ''); $safe=osv_video_scan_safe_uuid($appScanUuid,$scanId); $filename=(string)($video['filename'] ?? ''); $stem=$filename!==''?pathinfo($filename,PATHINFO_FILENAME):''; $base=$stem!==''?preg_replace('/_video$/','',$stem):''; $storagePath=(string)($video['storage_path'] ?? ''); $storageStem=$storagePath!==''?pathinfo($storagePath,PATHINFO_FILENAME):''; $storageBase=$storageStem!==''?preg_replace('/_video$/','',$storageStem):''; $candidates=[]; foreach([$safe,$stem,$base,$storageStem,$storageBase] as $prefix){ $prefix=trim((string)$prefix); if($prefix!==''){ $candidates[]=$dir.'/'.$prefix.'_camera_info.json'; } } foreach(array_values(array_unique($candidates)) as $path){ if(!is_file($path) || !is_readable($path) || filesize($path)<=0){ continue; } $decoded=json_decode((string)@file_get_contents($path),true); if(!is_array($decoded)){ continue; } $tof=$decoded['tof_capture_state'] ?? null; if(!is_array($tof)){ continue; } $frames=max(0,(int)($tof['frames_during_capture'] ?? 0)); if(!empty($tof['available']) && !empty($tof['active']) && $frames>0){ return ['active'=>true,'frames'=>$frames]; } } return $out; }
...
$autoPhotoSparseUiActionContext = [
  ...
  'csrf_name' => 'secCode',
  'csrf_value' => (string) ($_SESSION['secCode'] ?? ''),
];
$autoPhotoSparseUiRender=auto_photo_sparse_ui_render($autoPhotoSparseUi,$autoPhotoSparseUiActionContext);
