...
function osv_build_generated(array $sessions, array $merges=[]): array {
  ...
  foreach($merges as $m){ $type=(string)($m['merge_type'] ?? ''); $isAligned=($type==='aligned_shared_images_dense_ply'); $isManual=($type==='manual_correspondences_sim3_dense_ply'); $mid=(int)$m['id']; $rows[]=['source_type'=>'Merged','source_label'=>($isManual?'Manual aligned dense cloud #':($isAligned?'Aligned merged dense cloud #':'Diagnostic merged dense cloud #')).$mid,'mode'=>$isManual?'manual Sim(3) correspondences':($isAligned?'aligned shared images':'diagnostic merge'),'model_id'=>'-','job_id'=>$mid,'remote_job_id'=>0,'sparse_remote_job_id'=>0,'geometry_summary'=>(int)$m['total_points'].' points','merge_source_job_id'=>0,'open_dense_url'=>'/sfm_3d_viewer.php?order_id='.(int)$m['order_id'].'&merge_id='.$mid.'&artifact=dense','download_dense_url'=>'/api/sfm_generated_merge_file.php?merge_id='.$mid.'&file=ply','open_mesh_url'=>'','download_mesh_url'=>'','result_json_url'=>'/api/sfm_generated_merge_file.php?merge_id='.$mid.'&file=result','status_url'=>'/api/sfm_generated_merge_file.php?merge_id='.$mid.'&file=result']; }
  return $rows;
}
...
function osv_build_models_assemblies_by_session(array $sessions,array $merges,int $orderId): array {
  ...
  foreach($sessions as $s){
    ...
    foreach($merges as $m){ if((int)($m['capture_session_id'] ?? 0)!==$sid) continue; $meta=is_file((string)($m['result_json_path']??''))?(json_decode((string)file_get_contents((string)$m['result_json_path']),true)?:[]):[]; $items[]=['kind'=>'merge','badge'=>osv_merge_badge($m,$meta),'merge_id'=>(int)$m['id'],'merge_type'=>(string)$m['merge_type'],'status'=>(string)$m['status'],'source_jobs_json'=>(string)($m['source_jobs_json'] ?? ''),'points'=>(int)($m['total_points'] ?? 0),'scale'=>$meta['scale'] ?? ($meta['transform']['scale'] ?? null),'rms'=>$meta['rms'] ?? ($meta['rms_error'] ?? null),'pairs'=>$meta['pairs_count'] ?? (isset($meta['pairs'])&&is_array($meta['pairs'])?count($meta['pairs']):null),'method'=>$meta['method'] ?? ($meta['merge_method'] ?? ''),'open_url'=>'/sfm_3d_viewer.php?order_id='.$orderId.'&merge_id='.(int)$m['id'].'&artifact=dense','download_url'=>'/api/sfm_generated_merge_file.php?merge_id='.(int)$m['id'].'&file=ply','result_json_url'=>'/api/sfm_generated_merge_file.php?merge_id='.(int)$m['id'].'&file=result']; }
    $bySession[$sid]=$items;
  }
  return $bySession;
}
