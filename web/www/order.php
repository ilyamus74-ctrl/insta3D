...
function sfm_enrich_session_jobs(array $jobs): array {
  ...
  foreach($standalone as $si=>$sj){ if((string)$sj['job_type']==='COLMAP_SPARSE'){ $selection=[]; foreach(sfm_sparse_model_ids((int)$sj['remote_job_id']) as $mid){ $selection[$mid]=['label'=>'Not selected','class'=>'bg-secondary']; } foreach($parents as $rp){ if((int)($rp['parent_remote_job_id']??0)===(int)$sj['remote_job_id'] && $rp['ui_model_id']!==null){ $mid=(int)$rp['ui_model_id']; $label=((string)$rp['job_type']==='COLMAP_RECONSTRUCTION_HQ')?'Selected for HQ':'Selected for Preview'; $class=in_array(strtoupper((string)$rp['status']),$activeStatuses,true)?'bg-primary':'bg-info text-dark'; $selection[$mid]=['label'=>$label,'class'=>$class]; } } $standalone[$si]['sparse_model_selection']=$selection; } }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if (
    $action === 'auto_photo_sparse_export_ply'
    && $canDeleteMedia
) {
    try {
 ...
 if(in_array($action,['sfm_extract_frames_web','sfm_colmap_sparse_web','sfm_export_ply_web','sfm_colmap_dense_web','sfm_reconstruction_preview_web','sfm_reconstruction_hq_web','sfm_generate_mesh_preview_web','sfm_generate_mesh_hq_web'],true) && $canDeleteMedia){
   try{
     if($action==='sfm_extract_frames_web'){
       ...
       $colmap=(int)($_POST['colmap_job_id']??($_POST['sparse_remote_job_id']??0)); $model=(int)($_POST['model_id']??0); if($colmap<=0||$model<0){throw new RuntimeException('Bad COLMAP job or model id');}
       ...
       if(isset($_POST['best_model'])){ $model=sfm_best_sparse_model_id($colmap,sfm_sparse_model_ids($colmap)); } $parentJob=sfm_validate_sparse_model_for_order($dbcnx,$orderId,$colmap,$model); $captureSessionId=(int)$parentJob['capture_session_id']; $rid=sfm_job_id($dbcnx); $jt=$mode==='hq'?'COLMAP_RECONSTRUCTION_HQ':'COLMAP_RECONSTRUCTION_PREVIEW'; $out=sfm_remote_output_dir($rid).'/merged/merged_fused.ply'; $result=sfm_remote_output_dir($rid).'/merged/result.json'; $log=sfm_remote_output_dir($rid).'/logs'; $msg='chunked reconstruction queued';
...
$sfmJobsBySession=[];$denseJobsBySessionBundle=[];
$stmt=$dbcnx->prepare("SELECT * FROM sfm_remote_jobs WHERE order_id=? ORDER BY created_at DESC, id DESC");
if($stmt){ $stmt->bind_param('i',$orderId); $stmt->execute(); $rs=$stmt->get_result(); while($j=$rs->fetch_assoc()){ $sid=(int)$j['capture_session_id']; if(!isset($sfmJobsBySession[$sid])){$sfmJobsBySession[$sid]=[];} $j['status_url']='/api/sfm_remote_job_status.php?job_id='.(int)$j['id']; $j['status_json_url']='/api/sfm_remote_job_file.php?job_id='.(int)$j['id'].'&type=status'; $j['result_json_url']='/api/sfm_remote_job_file.php?job_id='.(int)$j['id'].'&type=result'; $j['logs_url']='/api/sfm_remote_job_file.php?job_id='.(int)$j['id'].'&type=logs'; $j['ply_url']=$j['status_url'].'&file=ply'; $j['mesh_final_url']=$j['status_url'].'&file=ply'; $j['mesh_poisson_url']=$j['status_url'].'&file=ply&mesh=poisson'; $j['mesh_cleaned_url']=$j['status_url'].'&file=ply&mesh=cleaned'; $j['dense_model_ids']=[0,1]; if((string)$j['job_type']==='MAKLERTOUR_SYNCED_DENSE'){ $base=(string)($j['output_path'] ?? ''); if($base===''){$base=sfm_remote_output_dir((int)$j['remote_job_id']);} $previewRel='dense/contact_dense_depth.jpg'; $debugRel='dense/dense_depth_debug.json'; $summaryRel='dense/dense_depth_summary.csv'; $j['preview_url']='/api/sfm_remote_job_artifact.php?job_id='.(int)$j['id'].'&file='.$previewRel; $j['debug_url']='/api/sfm_remote_job_artifact.php?job_id='.(int)$j['id'].'&file='.$debugRel; $j['summary_url']='/api/sfm_remote_job_artifact.php?job_id='.(int)$j['id'].'&file='.$summaryRel; $j['has_preview']=is_file(rtrim($base,'/').'/'.$previewRel); $j['has_debug']=is_file(rtrim($base,'/').'/'.$debugRel); $j['has_summary']=is_file(rtrim($base,'/').'/'.$summaryRel); $j['contact_dense_depth_url']=$j['has_preview']?$j['preview_url']:''; $j['dense_debug_url']=$j['debug_url']; $j['dense_summary_url']=$j['summary_url']; } $j['sparse_model_stats']=[]; $j['ui_model_id']=null; if((string)$j['job_type']==='COLMAP_MESH'){ $mr=sfm_remote_output_dir((int)$j['remote_job_id']).'/mesh/mesh_result.json'; $md=is_file($mr)?(json_decode((string)file_get_contents($mr),true)?:[]):[]; $mi=sfm_mesh_ply_info((int)$j['remote_job_id']); $j['mesh_engine']=$md['engine']??''; $j['mesh_vertices']=$md['vertices']??($md['mesh_vertices']??($mi['vertices']?:'')); $j['mesh_faces']=$md['faces']??($md['mesh_faces']??($mi['faces']?:'')); $j['mesh_mode']=$md['mode']??($j['reconstruction_mode']??''); $j['mesh_fallback']=!empty($md['fallback_used']); $j['mesh_duration_sec']=$md['duration_sec']??($md['duration']??''); $j['ui_can_download_mesh']=strtoupper((string)$j['status'])==='DONE' && $mi['downloadable']; } $params=json_decode((string)($j['parameters_json'] ?? '{}'), true); if(is_array($params) && array_key_exists('model_id',$params)){ $j['ui_model_id']=(int)$params['model_id']; } if((string)$j['job_type']==='COLMAP_SPARSE'){ $j['sparse_components']=sfm_sparse_components((int)$j['remote_job_id']); $j['dense_model_ids']=sfm_sparse_model_ids((int)$j['remote_job_id']); foreach($j['dense_model_ids'] as $mid){ $j['sparse_model_stats'][(int)$mid]=sfm_sparse_model_stats((int)$j['remote_job_id'],(int)$mid); } } if((string)$j['job_type']==='MAKLERTOUR_SYNCED_DENSE' && is_array($params) && isset($params['capture_bundle_id'])){ $bundleId=(int)$params['capture_bundle_id']; if($bundleId>0){ if(!isset($denseJobsBySessionBundle[$sid])){$denseJobsBySessionBundle[$sid]=[];} if(!isset($denseJobsBySessionBundle[$sid][$bundleId])){$denseJobsBySessionBundle[$sid][$bundleId]=[];} $j['ui_title']=sfm_job_title($j); $j['ui_progress_class']=sfm_job_status_class(strtoupper((string)($j['status'] ?? ''))); $j['ui_can_download_merged']=false; $j['ui_sparse_stats']=[]; $j['children']=[]; $denseJobsBySessionBundle[$sid][$bundleId][]=$j; } } $sfmJobsBySession[$sid][]=$j; } $stmt->close(); }
...
foreach($captureSessions as $idx=>$session){
  ...
  $sessionCaptureBundles=$captureBundlesBySession[(int)$session['id']] ?? []; foreach($sessionCaptureBundles as $cbIdx=>$cb){ $sessionCaptureBundles[$cbIdx]['synced_dense_jobs']=$denseJobsBySessionBundle[(int)$session['id']][(int)$cb['id']] ?? []; }
  $captureSessions[$idx]['capture_bundles']=$sessionCaptureBundles;
