...
SCRIPT="/home/makler/web/remote_station/scripts/align_dense_clouds_from_colmap_matches.py"
...
RUN_ID="order30_matches_$(date +%Y%m%d_%H%M%S)"
OUTPUT_DIR="/home/makler/web/remote_station/output/merged_order_30_colmap_matches_${RUN_ID}"

mkdir -p "$OUTPUT_DIR"

echo "Running raw COLMAP 3D↔3D match RANSAC"
...
python3 "$SCRIPT" \
  ...
  --iterations 80000 \
  --threshold-ratio 0.006 \
  --scale-min 0.05 \
  --scale-max 20 \
  --min-inliers 12 \
  --min-image-pair-coverage 4 \
  --seed 42 \
...
aligned = directory / "model1_aligned_to_model0_colmap_matches.ply"
merged = directory / "colmap_matches_merged_dense_cloud.ply"
...
print(
    "scale=",
    payload["transform_source_to_anchor"]["uniform_scale"],
)
print(
    "inliers=",
    payload["ransac"]["inlier_count"],
    "/",
    payload["ransac"]["unique_correspondences"],
)
print(
    "image_pair_coverage=",
    payload["ransac"]["image_pair_coverage"],
)
print(
    "median_residual=",
    payload["ransac"]["median_inlier_residual"],
)
...
echo
echo "Artifacts ready for review:"
echo "  $OUTPUT_DIR/colmap_matches_merged_dense_cloud.ply"
echo "  $OUTPUT_DIR/model1_aligned_to_model0_colmap_matches.ply"
echo "  $OUTPUT_DIR/merge_result.json"
echo "  $OUTPUT_DIR/inlier_correspondences.csv"
